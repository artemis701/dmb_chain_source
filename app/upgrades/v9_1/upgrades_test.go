	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("dmb", res))
				...
				suite.Require().Equal(balanceBefore.AmountOf("dmb"), sdk.NewDecFromInt(res))
			},
			true,
		},
		{
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expectedSuccess {
				for i := range v9.Accounts {
					...
					balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "dmb")
