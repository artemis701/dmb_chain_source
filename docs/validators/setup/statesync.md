...
```bash
ponted init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .ponted/config folder

```bash
mv $HOME/genesis.json ~/.ponted/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.ponted/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.ponted/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.ponted/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.ponted/config/config.toml
```

### Create ponted service

```bash
echo "[Unit]
Description=ponted Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which ponted) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/ponted.service; sudo mv $HOME/ponted.service /etc/systemd/system/
```

```bash
sudo systemctl enable ponted.service && sudo systemctl daemon-reload
```

### Run ponted

```bash
sytemctl start ponted
```

### Check logs

```bash
journalctl -u ponted -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.ponted/config/config.toml
```
