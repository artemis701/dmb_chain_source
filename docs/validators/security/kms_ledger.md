...
- Edit `addr` to point to your `ponted` instance.
- Adjust `chain-id` to match your `.ponted/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `ponted`.
...
The KMS may complain that it cannot connect to `ponted`. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.ponted/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `ponted`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell ponted the key that we've got in the previous section.

```bash
ponted gentx --name kmsval --pubkey <pub_key>
```

Now start `ponted`. You should see that the KMS connects and receives a signature request.
