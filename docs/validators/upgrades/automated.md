...
echo "export DAEMON_NAME=ponted" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.ponted" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.ponted`) and copy over the current binary.

```bash
mkdir -p ~/.ponted/cosmovisor
mkdir -p ~/.ponted/cosmovisor/genesis
mkdir -p ~/.ponted/cosmovisor/genesis/bin
mkdir -p ~/.ponted/cosmovisor/upgrades

cp $GOPATH/bin/ponted ~/.ponted/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `ponted` are the same:
...
```bash
sudo tee /etc/systemd/system/ponted.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.ponted"
Environment="DAEMON_NAME=ponted"
...
sudo -S systemctl enable ponted
sudo -S systemctl start ponted
```

You can check the status with:

```bash
systemctl status ponted
```
