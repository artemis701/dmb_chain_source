...
Before upgrading the Evmos version. Stop your instance of `ponted` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `ponted` is running.
:::
...
```bash
rm $HOME/.ponted/config/addrbook.json
