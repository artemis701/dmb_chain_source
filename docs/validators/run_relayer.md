...
First, set `grpc server` on port `9090` in the `app.toml` file from the `$HOME/.ponted/config` directory:

```bash
vim $HOME/.ponted/config/app.toml
```
...
Then, set the `pprof_laddr` to port `6060`, `rpc laddr` to port `26657`, and `prp laddr` to `26656` in the `config.toml` file from the `$HOME/.ponted/config` directory:

```bash
vim $HOME/.ponted/config/config.toml
```
