...
`ponted` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `ponted` binary, run:
...
::: tip
You can also enable auto-completion with the `ponted completion` command. For example, at the start of a bash session, run `. <(ponted completion)`, and all `ponted` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.ponted` directory.
...
To specify the `ponted` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.ponted/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.ponted/config/app.toml` file.
...
We can view the default client config setting by using `ponted config` command:
...
A list of commonly used flags of `ponted` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.ponted`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `ponted` commands. You can obtain the full list by using the `ponted -h` command.
