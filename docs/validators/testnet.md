...
```bash
ponted config chain-id evmos_9000-4
```
...
```bash
ponted init <your_custom_moniker> --chain-id evmos_9000-4
```
...
By default, the `init` command creates your `~/.ponted` (i.e `$HOME`) directory with subfolders `config/` and `data/`.
...
Check the `genesis.json` file from the [`archive`](https://archive.evmos.dev/evmos_9000-4/genesis.json) and copy it over to the `config` directory: `~/.ponted/config/genesis.json`. This is a genesis file with the chain-id and genesis accounts balances.

```bash
sudo apt install -y unzip wget
wget -P ~/.ponted/config https://archive.evmos.dev/evmos_9000-4/genesis.json
```
...
Your node needs to know how to find [peers](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#peers). You'll need to add healthy [seed nodes](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#seed) to `$HOME/.ponted/config/config.toml`. The [`testnets`](https://github.com/evmos/testnets) repo contains links to some seed nodes.

Edit the file located in `~/.ponted/config/config.toml` and the `seeds` to the following:
...
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" ~/.ponted/config/config.toml
```
...
We can set the [`persistent_peers`](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#persistent-peer) field in `~/.ponted/config/config.toml` to specify peers that your node will maintain persistent connections with. You can retrieve them from the list of
...
```bash
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" ~/.ponted/config/config.toml
```
...
```bash
ponted tx staking create-validator \
  --amount=1000000000000atevmos \
  --pubkey=$(ponted tendermint show-validator) \
  --moniker="EvmosWhale" \
  --chain-id=<chain_id> \
...
```bash
rm $HOME/.ponted/config/addrbook.json $HOME/.ponted/config/genesis.json
ponted tendermint unsafe-reset-all --home $HOME/.ponted
```
...
```bash
ponted tendermint show-node-id
```
