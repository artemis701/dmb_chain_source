...
```bash
ponted keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
ponted keys add --multisig=p1,p2,p3 --multisig-threshold=2 multisig_address
ponted keys add --multisig=p2,p3,p1 --multisig-threshold=2 multisig_address
```

Multisig addresses can also be generated on-the-fly and printed through the which command:

```bash
ponted keys show --multisig-threshold=K name1 name2 name3 [...]
```
...
```sh
ponted tx broadcast signedTx.json \
    --chain-id=evmos_9000-4 \
    --broadcast-mode=block
```
