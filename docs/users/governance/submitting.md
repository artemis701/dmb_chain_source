...
```json
{
  ...
  "deposit": "10000000000000000000dmb"
}
```
...
4. **Amount** - the amount of funding that the recipient will receive in atto-EVMOS (`dmb`)
5. **Deposit** - the amount that will be contributed to the deposit (in `dmb`) from the account submitting the proposal
...
The `amount` is `1000000000000000000dmb`. This is equal to 1 EVMOS, so `recipient` address `evmos1mx9nqk5agvlsvt2yc8259nwztmxq7zjq50mxkp` will receive 1 EVMOS if this proposal is passed.

The `deposit` of `192000000000000000000dmb` results in 192 EVMOS being used from the proposal submitter's account. A minimum deposit is required for a proposal to enter the voting period, and anyone may contribute to this deposit within 3 days. If the minimum deposit isn't reached before this time, the deposit amounts will be burned. Deposit amounts will also be burned if the quorum isn't met in the vote or the proposal is vetoed.
...
```json
{
  ...
  "deposit": "64000000000000000000dmb"
}
...
Users can query the proposal details with the `ponted` command-line interface using this command:

```bash
`ponted --node https://tendermint.bd.evmos.org:26657 query gov proposal 23`.
```
...
6. **Denom** - `dmb` (atto-EVMOS) will be the type of asset used as the deposit
7. **Amount** - the amount that will be contributed to the deposit (in `dmb`) from the account submitting the proposal
...
```bash
`ponted --node https://tendermint.bd.evmos.org:26657 query gov proposal 7`.
```
  ...
  "changes": [
    {
      "subspace": "gov",
      "key": "depositparams",
      "value": {"mindeposit":[{"denom":"dmb","amount":"64000000000000000000"}],
      "max_deposit_period":"1209600000000000"}
    }
  ],
  "deposit": "20100000000000000000dmb"
}
```

The deposit `denom` is `dmb` and `amount` is `20100000000000000000`. Therefore, a deposit of 20.1 EVMOS will be included with this proposal. At the time, the EVMOS mainnet had a 10 EVMOS minimum deposit, so this proposal was put directly into the voting period (and subsequently passed). The minimum deposit amount is currently 192 EVMOS. There is a minimum deposit required for a proposal to enter the voting period, and anyone may contribute to this deposit within a 3-day period. If the minimum deposit isn't reached before this time, the deposit amounts will be burned.

## Sending the transaction that submits your governance proposal

For information on how to use `ponted` binary to submit an on-chain proposal through the governance module, please refer to the [quickstart](../../validators/quickstart/binary.md) documentation.

### CLI

This is the command format for using `ponted` (the command-line interface) to submit your proposal on-chain:

```bash
ponted tx gov submit-proposal \
  --title=<title> \
...
1. `ponted` is the command-line interface client that is used to send transactions and query Evmos
...
After posting your transaction, your command line interface (`ponted`) will provide you with the transaction's hash, which you can either query using `ponted` or by searching the transaction hash using [Mintscan](https://www.mintscan.io/evmos) or any block explorer.
...
```bash
ponted q gov proposals --depositor evmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
ponted tx gov deposit <proposal-id> <deposit> --from <name>
```
...
- the deposit denomination is in `'atevmos'` instead of `'dmb'`
