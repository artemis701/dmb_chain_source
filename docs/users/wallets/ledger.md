...
```
ponted keys add NAME --ledger
```

**Example:**

```
ponted keys add myledger --ledger
...
```
ponted tx bank send myledger evmos1hnmrdr0jc2ve3ycxft0gcjjtrdkncpmmkeamf9 100000pose --fees 2000pose --from myledger
```
...
- `msgs`: This is the content of the cosmos transaction, in this example, we need to make sure that we are using a MsgSend, and that the *to_address* is the one that we want to send the founds. Also, we can verify that we are actually sending *10000dmb* to that wallet.
