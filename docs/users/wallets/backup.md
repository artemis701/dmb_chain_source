...
```bash
$ ponted keys add dev0-restored --recover
...
```bash
ponted keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ ponted keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ ponted keys unsafe-import-eth-key dev0-imported ./dev0.export
