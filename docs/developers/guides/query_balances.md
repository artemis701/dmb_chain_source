...
- [`ponted` & Tendermint RPC](#ponted--tendermint-rpc)
- [JSON-RPC](#json-rpc)
- [gRPC](#grpc)
...
## `ponted` & Tendermint RPC

Upon [installation](../../validators/quickstart/installation.md) and [configuration](../../validators/quickstart/binary.md) of the Evmos Daemon, developers can query account balances using `ponted` with the following CLI command:

```bash
$ ponted query bank balances $EVMOSADDRESS --count-total=$COUNTTOTAL --height=$HEIGHT --output=$OUTPUT --node=$NODE
balances:
...
Details of non-native currencies (ie. not `dmb`) can be queried with the following CLI command:

```bash
$ ponted query erc20 token-pair $DENOM --node=$NODE --height=$HEIGHT --output=$OUTPUT
token_pair:
...
Developers can query account balances of `dmb` using the [`eth_getBalance`](../json-rpc/endpoints.md#ethgetbalance) JSON-RPC method in conjunction with [`curl`](https://curl.se/):
