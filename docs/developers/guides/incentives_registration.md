...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`ponted`](../../validators/quickstart/binary.md), use the following command with `register-incentive`:
...
- `$ALLOCATION`: Denominations and percentage of the total rewards (25% of block distribution) to be allocated to users that interact and spend gas using the `$CONTRACTADDRESS` (eg. `"0.005000000000000000dmb"` will distribute 0.5% of out of the 25% tokens minted on each daily epoch rewards).
...
```bash
ponted tx gov submit-proposal register-incentive 0xFCd2Ce20ef8ed3D43Ab4f8C2dA13bbF1C6d9512F 0.050000000000000000pose 13 --description=$DESCRIPTION --title=$PROPOSALTITLE
```
...
```bash
ponted tx gov submit-proposal register-incentive --proposal=<path_to_json>
```
