...
This command creates a 4-node network using the `pontednode` Docker image.
...
Creating pontednode0 ... done
Creating pontednode2 ... done
Creating pontednode1 ... done
Creating pontednode3 ... done
```
...
calling the `ponted testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/ponted` directory in each container.
...
```bash
# node 0: daemon logs
docker exec pontednode0 tail ponted.log
...
To interact with `ponted` and start querying state or creating txs, you use the
`ponted` directory of any given node as your `home`, for example:

```bash
ponted keys list --home ./build/node0/ponted
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/ponted/key_seed.json` and can be restored to the CLI using the `ponted keys add --restore` command
:::
