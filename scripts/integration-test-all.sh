...
## default port prefixes for ponted
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/ponted keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/ponted init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/ponted add-genesis-account \
    "$("$PWD"/build/ponted keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000pose,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/ponted gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/ponted collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/ponted validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/ponted start --pruning=nothing --rpc.unsafe \
