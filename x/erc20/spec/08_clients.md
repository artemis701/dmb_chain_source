...
Find below a list of  `ponted` commands added with the  `x/erc20` module. You can obtain the full list by using the `ponted -h` command. A CLI command can look like this:

```bash
ponted query erc20 params
```
...
```bash
ponted tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
ponted tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
ponted tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
