...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.Apontedenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    claimstypes.DefaultClaimsDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.Apontedenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert dmb", func() {
			// Register 'dmb' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'dmb' thru IBC
			...
			// 1. Send dmb from Evmos to Osmosis
			...
			// check ibc dmb coins balance on Osmosis
			...
			// 2. Send dmb IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.Apontedenomtrace.Path, teststypes.Apontedenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.poseIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc dmb coins balance on Osmosis - should be zero
			...
			// check dmb balance after transfer - should be equal to initial balance
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			// should trigger claims logic and send dmb coins from claims to receiver
